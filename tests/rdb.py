...
def test_batch_endpoint():
    ...
    payload = [
        ...
        {
        "stmt": "INSERT INTO test_batch (name) VALUES (?);", "args": ['Alice']},
        {
...
def test_query_endpoint():
    ...
    payload = {
        "stmt": "SELECT * FROM test_batch where id <= ?;",
        "args": [3]
    }
    response = requests.post(url, json=payload, headers=headers)
    assert response.status_code == 200
...
def test_complicated_migration():
    ...
    jjson_payload = [{"stmt": stmt, "args": []} for stmt in payload]
    response = requests.post(url, json=jjson_payload, headers=headers)
    assert response.status_code == 200
