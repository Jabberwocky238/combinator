...
get_check_context() {
  ...
  local check_cross_layer=$(get_command_path "check-cross-layer")

  cat << EOF
{"file": "${finish_work}", "reason": "Finish work checklist"}
{"file": "$DIR_WORKFLOW/$DIR_SPEC/shared/index.md", "reason": "Shared coding standards"}
{"file": "${check_cross_layer}", "reason": "Cross-layer check spec"}
EOF
}
...
get_debug_context() {
  ...
  echo "{\"file\": \"${check_cross_layer}\", \"reason\": \"Cross-layer check spec\"}"
}
...
cmd_init_context() {
  ...
  if [[ -z "$target_dir" ]] || [[ -z "$dev_type" ]]; then
    ...
    echo "  dev_type: test | docs | feature"
...
cmd_create_pr() {
  ...
  local commit_prefix
  case "$dev_type" in
    feature|refactor) commit_prefix="feat" ;;
...
Examples:
  ...
  $0 init-context .trellis/tasks/01-21-add-login feature
  $0 init-context .trellis/tasks/01-21-add-login feature --platform cursor
  $0 add-context <dir> implement .trellis/spec/guides/auth.md "Auth guidelines"
