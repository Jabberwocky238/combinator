...
#
# Arguments:
#   project-type: general | docs | test (default: general)
#
# Prerequisites:
...
case "$PROJECT_TYPE" in
  general|docs|test)
    ;;
  *)
    echo -e "${YELLOW}Unknown project type: $PROJECT_TYPE, defaulting to general${NC}"
...
| `.trellis/spec/guides/code-reuse-thinking-guide.md` | Code reuse patterns and conventions |
EOF
}
...
write_task_json() {
  ...
  case "$project_type" in
    *)
      subtasks='[
    {"name": "Fill project guidelines", "status": "pending"},
