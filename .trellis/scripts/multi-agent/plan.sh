...
Arguments:
  ...
  --type, -t        Development type: feature | test | docs
  --requirement, -r Requirement description (quote if contains spaces)

Examples:
  $0 --name user-auth --type feature --requirement "Add JWT-based user authentication"
  $0 -n rate-limit -t feature -r "Add rate limiting to API endpoints"
...
if [[ ! "$DEV_TYPE" =~ ^(feature|test|docs)$ ]]; then
  log_error "Invalid dev type: $DEV_TYPE (must be: feature, test, docs)"
  exit 1
fi
